...
knife bootstrap windows winrm 10.2.0.4 --config c:\Users\chef-repo\.chef\knife.rb -x $adminusername  -P $adminPassword -N $wsfqdn
...
knife node run_list add --config c:\users\chef-repo\.chef\knife.rb --server-url https://$ChefServerFqdn/organizations/$organizationName/ $wsfqdn recipe[splunk-uf-install]
knife node run_list add --config c:\users\chef-repo\.chef\knife.rb --server-url https://$ChefServerFqdn/organizations/$organizationName/ $wsfqdn recipe[audit]
...
knife winrm name:${wsfqdn} -a ipaddress -x ${adminUsername}  -P $adminPassword --config c:\users\chef-repo\.chef\knife.rb chef-client
